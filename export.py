...
def comment_to_li(comment):
    ...
    html = "<h3>{0}: {1}</h3>".format(
        comment.get("author", "anonym"), comment.get("subject", "")
    )
    html += '\n<a id="comment-{0}"></a>'.format(comment["id"])

    if "body" in comment:
        html += "\n" + markdown.markdown(
            TAGLESS_NEWLINES.sub("<br>\n", htmlescape(comment["body"]))
        )
...
def save_as_markdown(id, subfolder, json_post, post_comments_html):
    ...
    with open(
        "posts-markdown/{0}/{1}.md".format(subfolder, id), "w", encoding="utf-8"
    ) as f:
        f.write(json_to_markdown(json_post))
    if post_comments_html:
        with open(
            "comments-markdown/{0}.md".format(json_post["slug"]), "w", encoding="utf-8"
        ) as f:
...
def save_as_html(id, subfolder, json_post, post_comments_html):
    ...
    with open(
        "posts-html/{0}/{1}.html".format(subfolder, id), "w", encoding="utf-8"
    ) as f:
...
def combine(all_posts, all_comments):
    ...
    for json_post in all_posts:
        ...
        post_comments = (
            jitemid in posts_comments and nest_comments(posts_comments[jitemid]) or None
        )
